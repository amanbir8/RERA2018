...
using CRUD.Models.DataAnalysisPanel;
...
namespace CRUD.Controllers.DataAnalysisPanel
{
    [Authorize]
    [Authorize(Roles = "SecretaryRERA, Programmer, Authority, ManagerDesk, PStoMembers, LegalAdvisorDesk, HelpDesk")]
    public class DataAnalysisPanelController : Controller
    {

        #region MIS Reports Dashboard
        [HttpGet]
        public ActionResult ReportsPanelDashboard()
        {
            return View("ReportsPanelDashboard");
        }

        [HttpGet]
        public ActionResult ReportsPanelAuthDashboard()
        {
            return View("ReportsPanelAuthDashboard");
        }

        [HttpGet]
        public ActionResult AnalysisReportingPanelInfoDesk()
        {
            return View("AnalysisReportingPanelInfoDesk");
        }

        [HttpGet]
        public ActionResult AnalysisReportingPanelSAInfoDesk()
        {
            return View("AnalysisReportingPanelSAInfoDesk");
        }

        [HttpGet]
        public ActionResult ReportsDashboardDefaultInfo()
        {
            //ClsPrp_AnalysisReportingPanel_View_InfoDesk aa = new ClsPrp_AnalysisReportingPanel_View_InfoDesk();
            ClsPrp_AuthorityDesk_AgentRERAnumberDetails aa = new ClsPrp_AuthorityDesk_AgentRERAnumberDetails();
            string prmUserRole = string.Empty;
            prmUserRole = getUserRole();
            string roletypeFlag = getMappingFlagValueforReportsPanel();
            aa.RoleAccessFlag = roletypeFlag;
            TempData["returnUrlCPvalue"] = getMappingUrlforReportsPanel(); TempData.Keep();
            return View("ReportsDashboardDefaultInfo", aa);
        }
        [HttpPost]
        public ActionResult ReportsDashboardDefaultInfo(ClsPrp_AuthorityDesk_AgentRERAnumberDetails smodel)
        {
            //ClsPrp_AnalysisReportingPanel_View_InfoDesk aa = new ClsPrp_AnalysisReportingPanel_View_InfoDesk();
            ClsPrp_AuthorityDesk_AgentRERAnumberDetails aa = new ClsPrp_AuthorityDesk_AgentRERAnumberDetails();
            ClsMethod_MIS_AgentQRcodeCertificateDetails sdb = new ClsMethod_MIS_AgentQRcodeCertificateDetails();
            ClsprpPanel_RegisteredAgentExportExcel aaXls = new ClsprpPanel_RegisteredAgentExportExcel();
            string prmUserRole = string.Empty;
            prmUserRole = getUserRole();
            string roletypeFlag = getMappingFlagValueforReportsPanel();
            aa.RoleAccessFlag = roletypeFlag;
            ...
            try
            {
                switch (smodel.IsRangeValueDateInputFlag)
                {
                    case 1:
                        ...
                        ModelState.Remove("RERAnumberRegistration");
                        ModelState.Remove("RERAnumberIssueDate");
                        ModelState.Remove("RERAnumberRegUptoDate");
                        break;
                    case 2:
                        ModelState.Remove("IsApplicationDateFlag");
                        ModelState.Remove("IsApplicationModeFlag");
                ...
                if (ModelState.IsValid)
                {
                    ...
                    flagAgentType = smodel.InputEntry_AgentType; //radioRangeDifferentMode
                    ...
                    switch (varprmRange)
                    {
                        case 1:
                            ...
                            varprmMonth = 0;
                            varprmYear = 0;
                            flagAgentType = smodel.InputEntry_AgentType;
                            break;
                        case 2:
                    ...
                    aa.prpongoing = sdb.Display_Panel_RegisteredAgentReports(UserID_Role, FromDateM, ToDateM, flagAgentType);

                    //ViewBag.data = "true";

                    foreach (var item in aa.prpongoing)
                    {
                        ClsprpPanel_RegisteredAgentExportExcel aaXlsInner = new ClsprpPanel_RegisteredAgentExportExcel();

                        //Real-Estate-Agent
                        aaXlsInner.AgentRegDiaryNumber_Name = item.AgentRegDiaryNumber_Name;
                        aaXlsInner.AgentDiaryNumberApplication_Date = item.CreatedOn;
                        aaXlsInner.AgentName = item.Agent_Name;
                        aaXlsInner.Agent_AddressDistrictName = item.Agent_AddressDistrictName;

                        //Details Of Name
                        aaXlsInner.Agent_FirstName = item.Agent_FirstName;
                        aaXlsInner.Agent_MiddleName = item.Agent_MiddleName;
                        aaXlsInner.Agent_LastName = item.Agent_LastName;
                        aaXlsInner.Organization_Name = item.Organization_Name;
                        aaXlsInner.Agent_Type = Convert.ToInt32(item.Agent_Type);
                    ...
                    Session["modelPanelRealEstateAgent"] = aaXls.prpongoing;
        ...
        #endregion
        public void RegisteredAgentsExportToExcel()
        {
            var objXlslist = Session["modelPanelRealEstateAgent"] as List<ClsprpPanel_RegisteredAgentExportExcel>;
            ...
            workSheet.Cells[1, 8].Value = "Agent First Name";
            workSheet.Cells[1, 9].Value = "Agent Middle Name";
            workSheet.Cells[1, 10].Value = "Agent Last Name";
            workSheet.Cells[1, 11].Value = "Organistation Name";
            workSheet.Cells[1, 12].Value = "Email Address";

            workSheet.Cells[1, 13].Value = "Mobile or Phone Number";
            workSheet.Cells[1, 14].Value = "Registration Number";
            workSheet.Cells[1, 15].Value = "RERA Number Issue Date";
            workSheet.Cells[1, 16].Value = "RERA Number Upto Date";
            ...
            foreach (var XlscodeItem in objXlslist)
            {
                ...
                workSheet.Cells[recordIndex, 2].Value = XlscodeItem.AgentRegDiaryNumber_Name;
                workSheet.Cells[recordIndex, 3].Value = XlscodeItem.AgentDiaryNumberApplication_Date.HasValue ? (XlscodeItem.AgentDiaryNumberApplication_Date != Convert.ToDateTime("0001-01-01 00:00:00") ? XlscodeItem.AgentDiaryNumberApplication_Date.Value.ToString("dd-MMM-yyyy") : string.Empty) : string.Empty;
                workSheet.Cells[recordIndex, 4].Value = XlscodeItem.AgentName;
                workSheet.Cells[recordIndex, 5].Value = XlscodeItem.Agent_AddressDistrictName;
                if (XlscodeItem.Agent_Type == 1)
                {
                    workSheet.Cells[recordIndex, 6].Value = "Individual";
                }
                else
                {
                    workSheet.Cells[recordIndex, 6].Value = "Other Than Individual(OTI)";
                }
                workSheet.Cells[recordIndex, 7].Value = XlscodeItem.Project_Name;

                workSheet.Cells[recordIndex, 8].Value = XlscodeItem.Agent_FirstName;
                workSheet.Cells[recordIndex, 9].Value = XlscodeItem.Agent_MiddleName;
                workSheet.Cells[recordIndex, 10].Value = XlscodeItem.Agent_LastName;
                workSheet.Cells[recordIndex, 11].Value = XlscodeItem.Organization_Name;
                workSheet.Cells[recordIndex, 12].Value = XlscodeItem.EmailAddress;
                workSheet.Cells[recordIndex, 13].Value = XlscodeItem.MobileNumber;

                workSheet.Cells[recordIndex, 14].Value = XlscodeItem.RERAnumberRegistration;
                workSheet.Cells[recordIndex, 15].Value = XlscodeItem.RERAnumberIssueDate.HasValue ? (XlscodeItem.RERAnumberIssueDate != Convert.ToDateTime("0001-01-01 00:00:00") ? XlscodeItem.RERAnumberIssueDate.Value.ToString("dd-MMM-yyyy") : string.Empty) : string.Empty;
                workSheet.Cells[recordIndex, 16].Value = XlscodeItem.RERAnumberRegUptoDate.HasValue ? (XlscodeItem.RERAnumberRegUptoDate != Convert.ToDateTime("0001-01-01 00:00:00") ? XlscodeItem.RERAnumberRegUptoDate.Value.ToString("dd-MMM-yyyy") : string.Empty) : string.Empty;
            ...
            using (var memoryStream = new MemoryStream())
            {
                ...
                Response.Charset = "";
                Response.AddHeader("content-disposition", "attachment; filename=ListofRegisteredAgents_" + strDateFormat + ".xls");
        ...
        private string getMappingUrlforReportsPanel()
        {
            ...
            vret = "DataAnalysisPanel/ReportsPanelAuthDashboard";
            // System Analysist
            if (pUserRole == "e583ee7f-aaa3-49bd-afc8-79c877afe591")
            {
                vret = "DataAnalysisPanel/AnalysisReportingPanelSAInfoDesk";
            }
            // Programmer
            if (pUserRole == "319a2b07-788f-4f90-9cda-3bca6ed81d3c")
            {
                vret = "DataAnalysisPanel/AnalysisReportingPanelInfoDesk";
            }
            // Secretary
            if (pUserRole == "15ac7786-a35e-46a3-ae19-8befd003dcfd")
            {
                vret = "DataAnalysisPanel/ReportsPanelDashboard";
            }
            // Authority
            if (pUserRole == "21f8f78b-5d55-4138-a503-57fcace0800e")
            {
                vret = "DataAnalysisPanel/ReportsPanelAuthDashboard";
            }
            return vret;
        ...
        private string getMappingFlagValueforReportsPanel()
        {
            string vret = string.Empty;
            string pUserRole = string.Empty;
            string pUser_WhoIdentified = string.Empty;
