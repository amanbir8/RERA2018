...
namespace CRUD.Models.ProjectPmPayment
{
    public static class ProjectPmPaymentReverifyService
    {
        public static PaymentGatewayReverifyResult ReverifyTransaction(string zPG_Transaction_ID)
        {
            ...
            var sdb = new ClsMethod_ProjectPmApplication_PaymentIntegration();
            ...
            try
            {
                ...
                switch (udf2)
                {
                    case "Project Application Fee (Registration)":
                        dbStatus = sdb.Update_ProjectPaymentGatewayByAPI(zPG_Transaction_ID, mihpayid, amount, bank_ref_num, pstatus, additionalcharges, bankcode, payment_type, payment_gateway, udf1, udf2, udf3, udf4, udf5);
                        break;
                    case "Project Application Fee (Extension of Registration)":
                        dbStatus = sdb.Update_ProjectExtensionPaymentGatewayByAPI(zPG_Transaction_ID, mihpayid, amount, bank_ref_num, pstatus, additionalcharges, bankcode, payment_type, payment_gateway, udf1, udf2, udf3, udf4, udf5);
                        break;
                    default:
        ...
        public static PaymentGatewayReverifyResult ReverifyEpayTransaction(string gatewayTxnID, int gatewayVal)
        {
            var result = new PaymentGatewayReverifyResult { MessageType = "info" };
            var sdb = new ClsMethod_ProjectPmApplication_PaymentIntegration();
            ...
            try
            {
                string key = gatewayVal == 1002? ConfigurationManager.AppSettings["payuMerchantKeyEpay"]: ConfigurationManager.AppSettings["payuMerchantKey"];
                string salt = gatewayVal == 1002? ConfigurationManager.AppSettings["payuSaltEpay"]: ConfigurationManager.AppSettings["payuSalt"];
                string uri = ConfigurationManager.AppSettings["payuApiBaseURL"];

                dynamic array = PayuVerifyClient.Verify(gatewayTxnID, key, salt, uri);
                string status = array["status"];
