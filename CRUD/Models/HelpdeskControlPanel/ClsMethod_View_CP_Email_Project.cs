...
{
    public class ClsMethod_View_CP_Email_Project
    {
        ...
        public List<ClsPrp_ControlPanel_View_Email_Project> Display_CP_RegisteredProjectDetailsForEmailAddress_ByID(string pRegistrationNumber, string pUserRole)
        {
            connection();
            List<ClsPrp_ControlPanel_View_Email_Project> CPregistrationList = new List<ClsPrp_ControlPanel_View_Email_Project>();
            ...
            foreach (DataRow dr in dt.Rows)
            {
                CPregistrationList.Add(
                    new ClsPrp_ControlPanel_View_Email_Project
                    {
        ...
        public List<ClsPrp_ControlPanel_View_Email_Project> Display_CP_EmailsAll_ForRegisteredProject_ByID(string pRegistrationNumber, string pUserRole)
        {
            connection();
            List<ClsPrp_ControlPanel_View_Email_Project> CPregistrationList = new List<ClsPrp_ControlPanel_View_Email_Project>();

            MySqlCommand cmd = new MySqlCommand("Display_RERA_CP_EmailsProjectAllDetails_ByID", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("p_RegistrationNumber", pRegistrationNumber);
            cmd.Parameters.AddWithValue("p_UserRole", pUserRole);
            MySqlDataAdapter sd = new MySqlDataAdapter(cmd);
            ...
            foreach (DataRow dr in dt.Rows)
            {
                CPregistrationList.Add(
                    new ClsPrp_ControlPanel_View_Email_Project
                    {
                        AdditionalProjectEmail_IndexID = Convert.ToInt64(dr["AdditionalProjectEmail_IndexID"]),
                        AdditionalProjectEmail_ID = Convert.ToInt64(dr["AdditionalProjectEmail_ID"]),
        ...
        public Int32 Update_LockUnlockHandler_CP_EmailAddressAll_ForRegisteredProjectsByIndex(string RequestID, string RelatedRefIndexID, string RelatedRefID, string RelatedCode, string LockUnlockCode, string ByUserName, string ByUserID, string RelatedRemarksIfAny, string RelatedPVMsg)
        {
            connection();
            MySqlCommand cmd = new MySqlCommand("usp_cp_setlockunlock_projectemailaddressByIndex", con);
            cmd.CommandType = CommandType.StoredProcedure;            
        ...
        public Tuple<bool, string> Add_ProjectRegisteredProject_EmailAddress(ClsPrp_ControlPanel_View_Email_Project smodel, string User_Name, Int64 sProject_ID, Int64 sPromoter_ID, string sProjectDiaryNumber, string sUID)
        {
            connection();
            MySqlCommand cmd = new MySqlCommand("usp_insert_CP_tbl_rera_project_additionalemailaddress_project", con);
            cmd.CommandType = CommandType.StoredProcedure;

            DateTime dtvalue = new DateTime(0001, 1, 1);

            #region Parameters
            cmd.Parameters.AddWithValue("p_AdditionalProjectEmail_IndexID", (smodel.AdditionalProjectEmail_IndexID == 0) ? 0 : smodel.AdditionalProjectEmail_IndexID);
            cmd.Parameters.AddWithValue("p_AdditionalProjectEmail_ID", (smodel.AdditionalProjectEmail_ID == 0) ? 0 : smodel.AdditionalProjectEmail_ID);
