...
{
    public class ClsMethod_View_CP_Mobile_Project
    {
        ...
        public List<ClsPrp_ControlPanel_View_Mobile_Project> Display_CP_RegisteredProjectDetailsForMobileNumber_ByID(string pRegistrationNumber, string pUserRole)
        {
            connection();
            List<ClsPrp_ControlPanel_View_Mobile_Project> CPregistrationList = new List<ClsPrp_ControlPanel_View_Mobile_Project>();
            ...
            foreach (DataRow dr in dt.Rows)
            {
                CPregistrationList.Add(
                    new ClsPrp_ControlPanel_View_Mobile_Project
                    {
        ...
        public List<ClsPrp_ControlPanel_View_Mobile_Project> Display_CP_MobileNumbersAll_ForRegisteredProject_ByID(string pRegistrationNumber, string pUserRole)
        {
            connection();
            List<ClsPrp_ControlPanel_View_Mobile_Project> CPregistrationList = new List<ClsPrp_ControlPanel_View_Mobile_Project>();

            MySqlCommand cmd = new MySqlCommand("Display_RERA_CP_MobileProjectNumbersAllDetails_ByID", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("p_RegistrationNumber", pRegistrationNumber);
            cmd.Parameters.AddWithValue("p_UserRole", pUserRole);
            MySqlDataAdapter sd = new MySqlDataAdapter(cmd);
            ...
            foreach (DataRow dr in dt.Rows)
            {
                CPregistrationList.Add(
                    new ClsPrp_ControlPanel_View_Mobile_Project
                    {
                        AdditionalProjectMobileNumber_IndexID = Convert.ToInt64(dr["AdditionalProjectMobileNumber_IndexID"]),
                        AdditionalProjectMobileNumber_ID = Convert.ToInt64(dr["AdditionalProjectMobileNumber_ID"]),
        ...
        public Int32 Update_LockUnlockHandler_CP_MobileNumbersAll_ForRegisteredProjectsByIndex(string RequestID, string RelatedRefIndexID, string RelatedRefID, string RelatedCode, string LockUnlockCode, string ByUserName, string ByUserID, string RelatedRemarksIfAny, string RelatedPVMsg)
        {
            connection();
            MySqlCommand cmd = new MySqlCommand("usp_cp_setlockunlock_projectmobilenumberByIndex", con);
            cmd.CommandType = CommandType.StoredProcedure;            
        ...
        public Tuple<bool, string> Add_ProjectRegisteredProject_PhoneMobileNumber(ClsPrp_ControlPanel_View_Mobile_Project smodel, string User_Name, Int64 sProject_ID, Int64 sPromoter_ID, string sProjectDiaryNumber, string sUID)
        {
            connection();
            MySqlCommand cmd = new MySqlCommand("usp_insert_CP_tbl_rera_project_additionalmobilenumber_project", con);
            cmd.CommandType = CommandType.StoredProcedure;

            DateTime dtvalue = new DateTime(0001, 1, 1);

            #region Parameters
            cmd.Parameters.AddWithValue("p_AdditionalProjectMobileNumber_IndexID", (smodel.AdditionalProjectMobileNumber_IndexID == 0) ? 0 : smodel.AdditionalProjectMobileNumber_IndexID);
            cmd.Parameters.AddWithValue("p_AdditionalProjectMobileNumber_ID", (smodel.AdditionalProjectMobileNumber_ID == 0) ? 0 : smodel.AdditionalProjectMobileNumber_ID);
