...
{
    public class ClsMethod_View_CP_Mobile_ComplaintFormM
    {
        ...
        public List<ClsPrp_ControlPanel_View_Mobile_ComplaintFormM> Display_CP_FormMcomplaintDetailsForMobileNumber_ByID(string pRegistrationNumber, string pUserRole)
        {
            connection();
            List<ClsPrp_ControlPanel_View_Mobile_ComplaintFormM> CPregistrationList = new List<ClsPrp_ControlPanel_View_Mobile_ComplaintFormM>();
            ...
            foreach (DataRow dr in dt.Rows)
            {
                CPregistrationList.Add(
                    new ClsPrp_ControlPanel_View_Mobile_ComplaintFormM
                    {
        ...
        public List<ClsPrp_ControlPanel_View_Mobile_ComplaintFormM> Display_CP_MobileNumbersAll_ForRegisteredComplaintFormM_ByID(string pRegistrationNumber, string pUserRole)
        {
            connection();
            List<ClsPrp_ControlPanel_View_Mobile_ComplaintFormM> CPregistrationList = new List<ClsPrp_ControlPanel_View_Mobile_ComplaintFormM>();

            MySqlCommand cmd = new MySqlCommand("Display_RERA_CP_MobileComplaintMFormNumbersAllDetails_ByID", con);
            cmd.CommandType = CommandType.StoredProcedure;
            cmd.Parameters.AddWithValue("p_RegistrationNumber", pRegistrationNumber);
            cmd.Parameters.AddWithValue("p_UserRole", pUserRole);
            MySqlDataAdapter sd = new MySqlDataAdapter(cmd);
            ...
            foreach (DataRow dr in dt.Rows)
            {
                CPregistrationList.Add(
                    new ClsPrp_ControlPanel_View_Mobile_ComplaintFormM
                    {
                        AdditionalMobileNumberFormM_IndexID = Convert.ToInt64(dr["AdditionalMobileNumberFormM_IndexID"]),
                        AdditionalMobileNumberFormM_ID = Convert.ToInt64(dr["AdditionalMobileNumberFormM_ID"]),
                        ComplaintFormM_ID = Convert.ToInt64(dr["ComplaintFormM_ID"]),
                        ComplaintFormM_Code = Convert.ToString(dr["ComplaintFormM_Code"]),
                        Profile_ID = Convert.ToInt64(dr["Profile_ID"]),
                        User_ID = Convert.ToString(dr["User_ID"]),
        ...
        public Int32 Update_LockUnlockHandler_CP_MobileNumbersAll_ForRegisteredComplaintMFormByIndex(string RequestID, string RelatedRefIndexID, string RelatedRefID, string RelatedCode, string LockUnlockCode, string ByUserName, string ByUserID, string RelatedRemarksIfAny, string RelatedPVMsg)
        {
            connection();
            MySqlCommand cmd = new MySqlCommand("usp_cp_setlockunlock_complaintmformmobilenumberByIndex", con);
            cmd.CommandType = CommandType.StoredProcedure;
        ...
        public Tuple<bool, string> Add_ComplaintFormM_PhoneMobileNumber(ClsPrp_ControlPanel_View_Mobile_ComplaintFormM smodel, string User_Name, Int64 sFormM_ID, string sFormM_Code, string sFormM_DiaryNumber, string sUID)
        {
            connection();
            MySqlCommand cmd = new MySqlCommand("usp_insert_CP_tbl_rera_complaint_formm_additionalmobilenumber", con);
            cmd.CommandType = CommandType.StoredProcedure;

            DateTime dtvalue = new DateTime(0001, 1, 1);

            #region Parameters
            cmd.Parameters.AddWithValue("p_AdditionalMobileNumberFormM_IndexID", (smodel.AdditionalMobileNumberFormM_IndexID == 0) ? 0 : smodel.AdditionalMobileNumberFormM_IndexID);
            cmd.Parameters.AddWithValue("p_AdditionalMobileNumberFormM_ID", (smodel.AdditionalMobileNumberFormM_ID == 0) ? 0 : smodel.AdditionalMobileNumberFormM_ID);
